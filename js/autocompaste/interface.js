...
AutoComPaste.Interface = (function () {
   ...
   */
  function Interface (wm, engine, texts_json, stimuli, window) {
    /** Internal functions */
      ...
      if (privates.texts_returned == privates.texts_available) {
        ...
        for (var text_title in privates.texts) {
          if (privates.texts.hasOwnProperty(text_title)) {
            ...
            var singleWindow = privates.window == 1;
            var tripleWindow = privates.window == 3;
            var hasStimuli = privates.texts[text_title].includes(privates.stimuli);

            console.log(hasStimuli);
            ...
            } else if(tripleWindow) {
                if(hasStimuli) {
                    iface._createWindowForText(text_title);
                } else {
                    if(windowCreated == 2) {
                    } else if (windowCreated == 1) {
                        windowCreated = 2;
                        iface._createWindowForText(text_title);
                    } else {
                        windowCreated = 1;
