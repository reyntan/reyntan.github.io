    ...
    if (window.location.pathname.indexOf('experiment') > -1) {
        ...
        module.presentTrial = function (options) {
            ...
            var iface = new AutoComPaste.Interface(wm, engine, data_file, stimuli, window);

            // Highlight the relevant text.
            iface.addEventListener('loaded', function () {
                var lines_to_highlight = stimuli.split("\n\n");

                var window = wm.getWindowList();
                for (var i = 0; i < window.length; i++) {
                    if (window[i] == 'text_editor') {
                        continue;
                    }

                    var win = wm.getWindowContent(window[i]);
